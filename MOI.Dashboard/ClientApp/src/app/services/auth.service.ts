...
// import { LayoutStore } from '../../../node_modules';
...
export class AuthService {
  ...
  constructor( private router: Router, private configurations: ConfigurationService,
    private endpointFactory: EndpointFactory, private localStorage: LocalStoreManager, private svc: CommonService) {
  ...
  async  processLoginResponse(response: LoginResponse, rememberMe: boolean) {
    ...
    window.localStorage.setItem('UserName', decodedIdToken.userName);
    const user = new User(
      decodedIdToken.sub,
    ...
    const leftNavigationItems = decodedIdToken['userNavigation'];
    // const configuration = JSON.parse(decodedIdToken.configuration);
    const configuration = [];
  ...
  public loadLeftNavigation() {
    const sidebarLeftMenu = JSON.parse(this.leftNavigationMenu);
  ...
  private saveUserDetails(user: User, permissions: PermissionValues[], accessToken: string, idToken: string,
     refreshToken: string, expiresIn: Date, leftNavigationItems: string, rememberMe: boolean) {
